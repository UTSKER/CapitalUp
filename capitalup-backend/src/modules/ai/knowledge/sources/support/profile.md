...
- **Overview**: Notifications are generated in real-time to alert users when limit orders are matched, stop-losses are triggered, KYC status updates occur, or deposits/withdrawals are finalized.
- **Endpoints**:
  - `GET /api/v1/notifications/`: Fetch all notifications for the authenticated user.
  - `PATCH /api/v1/notifications/:id/read`: Mark a specific notification (by ID) as read.
  - `PATCH /api/v1/notifications/read-all`: Mark all notifications for the current user as read.
- **Access Rule**: Requires authenticated user access.
